...
class Rectangle:
    ...
    print_symbol = "#" # Initialize the print_symbol class attribute to '#'
    ...
    def __str__(self):
        """
        Returns a string representation of the rectangle using the `print_symbol` character(s).
        If width or height is 0, returns an empty string.
        """
        if self.__width == 0 or self.__height == 0:
            return ""
        
        # Create a list of strings, where each string is a row of `print_symbol` characters
        # and then join them with newline characters.
        rectangle_str = []
        for _ in range(self.__height):
            # Convert print_symbol to string if it's not already, to allow multiplication
    ...
    def __del__(self):
        ...
        decrements the number_of_instances counter.
        """
        print("Bye rectangle...")
        Rectangle.number_of_instances -= 1 # Decrement instance count upon deletion
